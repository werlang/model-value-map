...
model listed on `opencode.ai/docs/go` always shows, automatically. The docs
send no CORS headers, so the browser never reads them directly: the worker
scrapes the Go table server-side and serves the ids at `GET /curated`
(CORS `*`), which `app.js` fetches as its only roster source. AA scores are
sourced in three tiers — the keyed free-tier API,
...
3. **Never execute remote content.** The worker parses Artificial Analysis payloads and the Go docs HTML as inert data with regex; the app only ever consumes validated JSON. Nothing parsed from a remote source is ever `eval`'d or injected as a script.
4. **Graceful degradation is mandatory.** Every live value passes validation before plotting; anything missing or offline falls back to cached entries. If the Go-table roster can't be fetched (worker `/curated` down), the full payload renders unfiltered — the page never reads `opencode.ai/docs/go` directly, since it sends no CORS headers. The page must always render, even fully offline.
...
- The roster is derived, never hard-coded: the worker parses the Go docs tables (a `Model ID` column preferred, else `Model`) plus the listed models, normalize with dots-to-dashes, and serves the ids at `GET /curated`. Every Go-table model must appear on the page (plotted or, honestly, in the off-map tray). `AA_SLUG` (`worker/index.js`) holds curated slug overrides — add entries only when normalization can't match.
...
Go-table roster filter (worker `/curated` only — the CORS-blocked docs are
never requested client-side, off-table exclusion), Pareto ties, and the
render/stamp behavior of a booted page. `app.js` exposes
